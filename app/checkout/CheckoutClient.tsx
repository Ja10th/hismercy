...
import { ArrowLeft, ChevronDown, Mail, Phone, User } from "lucide-react";
...
export default function CheckoutClient({ deliverySettings }: CheckoutClientProps) {
  ...
  const originState = deliverySettings.originState?.trim() || ORIGIN_STATE;
  const originCity = deliverySettings.originCity?.trim() || ORIGIN_CITY;

  const rates = useMemo(
    () => buildDeliveryRates(deliverySettings),
    [deliverySettings],
  );

  const deliveryZone = useMemo(() => {
    return getDeliveryZone(form.state, form.city, originState, originCity);
  }, [form.state, form.city, originState, originCity]);

  const selectedRate = rates[deliveryZone];

  const pickupEnabled = deliverySettings.pickupEnabled ?? true;

  useEffect(() => {
    if (!pickupEnabled && deliveryMethod === "pickup") {
      setDeliveryMethod("delivery");
    }
  }, [pickupEnabled, deliveryMethod]);

  const deliveryFee = useMemo(() => {
    if (deliveryMethod === "pickup") return 0;
    return selectedRate.feePerBag * bagCount;
  }, [deliveryMethod, selectedRate, bagCount]);

  const deliveryRuleMessage = useMemo(() => {
    if (deliveryMethod === "pickup") return null;

    const minimum = selectedRate.minBags;
    if (bagCount < minimum) {
      return `Minimum order for ${selectedRate.label} is ${minimum} bags. You currently have ${bagCount} bag${bagCount === 1 ? "" : "s"}.`;
    }
